    ...
    bot.on("message:document", async (ctx) => {
      ...
      if (doc) {
        try {
          ...
          if (!file) return;
          attachments = [
            { url: `https://api.telegram.org/file/bot${this.token}/${file.file_path}`, name: doc.file_name ?? "file" },
          ];
        } catch {}
      }
      this.messageHandler({
    ...
    return new Promise((resolve) => {
      ...
      const timer = setTimeout(
        async () => {
          ...
          await this.bot?.api
            .editMessageText(Number(this.chatId), msg?.message_id ?? 0, "Permission: Timed out (denied)")
            .catch(() => {});
          resolve("reject");
        },
        5 * 60 * 1000,
