...
import type { OpenChannelsConfig, PluginState, QueuedMessage, StreamingState } from "./types";
...
const _debug =
  process.env.OPENCODE_LINK_DEBUG === "1"
    ? (dir: string, ...args: any[]) => {
        const ts = new Date().toISOString();
        const msg = args.map((a) => (typeof a === "string" ? a : JSON.stringify(maskSensitive(a)))).join(" ");
        try {
          appendFileSync(join(dir, ".opencode", "opencode-link-debug.log"), `[${ts}] ${msg}\n`);
        } catch {}
      }
    : () => {};
...
async function throttledEdit(
  ...
  const display = allText.length > maxLen ? `${allText.slice(0, maxLen)}\n...` : allText;
...
export const openChannels: Plugin = async ({ client, directory, serverUrl: _serverUrl }) => {
  ...
  return {
    ...
    "permission.ask": (input, output) => handlePermission(input, output, state, config, client),
  };
};
...
async function processMessage(
  ...
  try {
    ...
    if (!sessionId) {
      ...
      state.sessionMap.set(msg.channelId, sessionId as string);
    }
    ...
    const parts: Array<{ type: "text"; text: string }> = [{ type: "text", text: msg.content }];
    ...
    try {
      ...
      await state.provider.editMessage(
        msg.channelId,
        placeholderId,
        `**Error:** ${promptErr.message ?? String(promptErr)}`,
      );
      return;
    }
...
async function handleEvent(event: any, state: PluginState, config: OpenChannelsConfig, _sdkClient: any): Promise<void> {
  ...
  if (event.type === "session.error") {
    ...
    const errorMsg = event.properties?.error?.data?.message ?? event.properties?.error?.message ?? "Unknown error";
...
async function handleCommand(ctx: CommandContext, state: PluginState, sdkClient: any): Promise<void> {
  if (!state.ready) return;
  ...
  try {
    switch (ctx.commandName) {
      ...
      case "sessions": {
        ...
        const lines = sessions.slice(0, 10).map((s: any) => `- \`${s.id}\` ${s.title ?? "untitled"}`);
